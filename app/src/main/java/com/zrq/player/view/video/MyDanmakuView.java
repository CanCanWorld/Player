...
//                .setCacheStuffer(new BackgroundCacheStuffer(), null)  // 绘制背景使用BackgroundCacheStuffer
    ...
    private final Map<Integer, String> mDanmakuPool = new TreeMap<>();
    private boolean isShowDanmaku = false;

    //设置弹幕池
    public void setDanmakuPool(Map<Integer, String> danmakuPool) {
        mDanmakuPool.clear();
        mDanmakuPool.putAll(danmakuPool);
    }

    //获取弹幕池
    public Map<Integer, String> getDanmakuPool() {
        return mDanmakuPool;
    }
    ...
    //开始显示弹幕(根据弹幕池)，传参： 时间戳、
    public void startDanmaku(Integer time) {
        isShowDanmaku = true;
        if (mDanmakuPool.size() == 0) return;
        new Thread(() -> {
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N) {
                mDanmakuPool.forEach((integer, s) -> {
                    if (isShowDanmaku) {
                        if (integer < time) return;
                        try {
                            Thread.sleep((long) integer - lastTime);
                            Log.d(TAG, "time: " + integer + ", msg: " + s);
                            lastTime = integer;
                            addDanmaku(s, false, 0L);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
     ...
     */
    public void addDanmaku(String text, boolean isSelf, long time) {
     ...
     * 发送自定义弹幕
     */
    public void addDanmakuWithDrawable(Drawable drawable) {
